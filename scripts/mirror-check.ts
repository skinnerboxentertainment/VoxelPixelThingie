...
if (!dir || urls.length === 0) {
  console.error("usage: mirror-check <folder> <url> [url...]");
  process.exit(2);
}
...
async function check(name: string, store: NodeFsStore | FetchStore): Promise<void> {
...
await check("local", new NodeFsStore(dir));
for (const [i, url] of urls.entries()) await check(`url${i + 1}`, new FetchStore(url));
