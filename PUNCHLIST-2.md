...
| 13 | **A shared cube: controller here, cube away.** One scene record both sides write to and read from; the scene server as the control surface on one side, the worker and the LED driver on the other; senses back the same road; both ends compute the digest and compare. | A change made through the controller lands in the shared ledger under the controller's name and lights the far cube (the twin until #72); a reading from the far side appears at the controller; both ends report the same digest; the record survives either side going down. Where the record lives is a decision: the scenes repository (seconds of lag, every version sealed) or a small always-on host (the first account and bill in the project). | M |
| 14 | **The embedded assembler.** Programs carried as readable WebAssembly text in the passport and assembled inside the box on load, so a bit's behavior can be read by a person as well as run; and, behind its own policy word, a program that may propose growing a bit into an empty neighbor cell, so a swarm can build. | A program carried as text assembles to the same bytes and the same CID on every host; the passport shows the text and the reader displays it; a `grow` op refused by a bit without the policy word; with it, a seed grows a fixture shape in a fixed number of rounds with the same digest on three hosts. | L, after PLAN-5 Phases 30 and 31 |

Items 11 and 12 were added the same evening at Oscar's request and are
planned in PLAN-5.md with item 6. Items 13 and 14 were captured later the
same night while thinking aloud, before a break, so they would not be lost;
neither is planned.
